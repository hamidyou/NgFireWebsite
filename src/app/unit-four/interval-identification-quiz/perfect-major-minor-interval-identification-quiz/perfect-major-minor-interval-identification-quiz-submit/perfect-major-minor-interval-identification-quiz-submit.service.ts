...
@Injectable()
export class PerfectMajorMinorIntervalIdentificationQuizSubmitService {
  ...
  quizSet(): void {
    ...
    this.postUnderQuiz = this.db.list('quizzes/PerfectMajorMinorIntervalIdentificationQuiz/' + this.userFile.school + '/' + this.userFile.lastname + this.userFile.firstname);

    this.postUnderLastName = this.db.list('/quizzes/' + this.userFile.school + '/' + this.userFile.lastname + this.userFile.firstname + '/' + 'PerfectMajorMinorIntervalIdentificationQuiz');
  ...
  submitScore(): void {
    if (this._intervalIdentificationQuizzesService.qualitiesCorrect < 12) {
      this.verify = 'This quiz cannot be submitted until you have 12 quantities and qualities correct. Please click' +
      ...
      this.currentUser = this.db.object('users/' + this.userFile.school + '/' + this.userFile.program + '/' + this.userFile.lastname + this.userFile.firstname + '/quizScores/PerfectMajorMinorIntervalIdentificationQuiz').take(1);
            ...
            this.currentUser.subscribe(data => {
              if (data.$exists) {
                this.currentUser.set({
                  'PerfectMajorMinorIntervalIdentificationQuiz': {
                    dateTimeSubmitted: new Date().toLocaleString(),
                    timeElapsed: this.timeElapsed,
                    total: (this._intervalIdentificationQuizzesService.total * 100).toFixed(0),
                    quantitiesAttempted: this._intervalIdentificationQuizzesService.quantitiesAttempted,
