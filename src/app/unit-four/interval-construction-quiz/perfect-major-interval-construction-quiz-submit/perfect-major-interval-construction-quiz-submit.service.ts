...
@Injectable()
export class PerfectMajorIntervalConstructionQuizSubmitService {
  ...
  quizSet(): void {
    ...
    this.postUnderQuiz = this.db.list('quizzes/PerfectMajorIntervalConstructionQuiz/' + this.userFile.school + '/' + this.userFile.lastname + this.userFile.firstname);

    this.postUnderLastName = this.db.list('/quizzes/' + this.userFile.school + '/' + this.userFile.lastname + this.userFile.firstname + '/' + 'PerfectMajorIntervalConstructionQuiz');
  ...
  submitScore(): void {
    if (this._IntervalConstructionQuizService.answerNotesCorrect < 10) {
      ...
      this.currentUser = this.db.object('users/' + this.userFile.school + '/' + this.userFile.program + '/' + this.userFile.lastname + this.userFile.firstname + '/quizScores/PerfectMajorIntervalConstructionQuiz').take(1);
            ...
            this.currentUser.subscribe(data => {
              if (data.$exists) {
                this.currentUser.set({
                  'PerfectMajorIntervalConstructionQuiz': {
                    dateTimeSubmitted: new Date().toLocaleString(),
                    timeElapsed: this.timeElapsed,
                    total: (this._IntervalConstructionQuizService.total * 100).toFixed(0),
                    answerNotesAttempted: this._IntervalConstructionQuizService.answerNotesAttempted,
